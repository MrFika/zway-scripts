...
def login_request(session, login_header, login_payload):
        resp = session.post(host + api_path + 'login',
...
def main(argv):
        ...
        # Do the Login.
        login_payload = {"login": args.username,
                         "password": args.password,
                         "rememberme": 'true'}
        login_header = {'User-Agent': 'Mozilla/5.0',
        ...
        input_nbr = int(input("Enter ID number of the borrowed switch"
                              " (ID on the left): "))
        ...
        while True:
                ...
                print(get_request(
                    session,
                    "devices/" + switch_device[input_nbr]["id"] +
                    "/command/on"))
                ...
                iterations += 1
                print("Running for ~" + str(runtime) +
                      " seconds, " + str(iterations) +
                      "on/off iterations")
